...
export const useTeachersStore = defineStore('teachers', () => {
  ...
  const selectAll = computed(() => {
    const totalSelected = Object.keys(selected.value).length;
    if (!totalSelected){return false}
    const selectedOnPage = currentRecords.value.filter((row) =>{
      return selected.value[row.number] === true;
    }).length;

    return selectedOnPage === perPage.value;
  })
  ...
  async function selectAllRows(){
    if (selectAll.value){
      currentRecords.value.forEach((row) => {
        delete selected.value[row.number];
      });
      return;
    }
    currentRecords.value.forEach((row) => {
       selected.value[row.number] = true;
    })
  }
