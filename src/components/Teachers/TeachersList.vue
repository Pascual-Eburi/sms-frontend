...
const { currentRecords,selectAll,selected } = storeToRefs(teachersStore)
</script>
...
<template>
  <div class="card relative">
    <div class="card-body border-bottom py-3">
      <div class="d-flex">
        <PageLengthSelector :store="teachersStore" />
        <SearchInput :store="teachersStore" />
      </div>
    </div>
          ...
          <tr>
            <th class="w-1">
              <input
                ...
                type="checkbox"
                @change="teachersStore.selectAllRows()"
                :checked="selectAll"
          ...
          <tr v-for="line in currentRecords" :key="line.number">
            <td>
              <input
                class="form-check-input m-0 align-middle"
                type="checkbox"
                :checked="selected[line.number]"
                aria-label="Select invoice"
                @change="teachersStore.selectRow(line.number)"
              />
            </td>
            <td>
    ...
    <div class="card-footer d-flex align-items-center">
      <PaginationInfo :store="teachersStore" />
      <PaginationPages :store="teachersStore" />
    </div>
    <ActionToolbar />
  </div>
</template>
